...
class SpaceShip(SpaceObject):
	...
	def update_player_postion_mount(self, screen_sizeX, screen_sizeY):

		# Update X position (update with min/max)
		self.posX = screen_sizeX
		
		# Update Y position (update with min/max)
		self.posY = screen_sizeY
...
PLAYER_NAME	= 'DAN'		#sws.PLAYER_NAME	# 'DAN'
		...
		for event in pygame.event.get():	
			...
			if event.type == pygame.KEYDOWN:
				...
				# 'arrow keys' for movement
					...
					player.speedX = -player_maxSpeedX*2
					keyX_pressed += 1
				elif event.key == pygame.K_RIGHT:
					player.speedX = player_maxSpeedX*2
					keyX_pressed += 1
				elif event.key == pygame.K_UP:
					player.speedY = -player_maxSpeedY*2
					keyY_pressed += 1
			...
			if (event.type == pygame.MOUSEBUTTONDOWN ) and bullet.state == 'hide':
				bullet.fire_bullet(player)

		# Move player and check not out of screen
		mx , my = pygame.mouse.get_pos()
		player.update_player_postion_mount(mx, my)
		#print(f'x : {mx} y : {my}')
